...
let FIFO_ROCK_UNROCK = './rock_unrock.fifo';
...
exports.init = function(server) {
	...
	exec('mkfifo ' + FIFO_ROCK_UNROCK, function(err, stdout, stderr){});
	...
	io.sockets.on('connection', function(client) {
		...
		client.on('rock_unrock/update', function(d){
			console.log('rock_unrock/update ' + d.rock_state)
			var wstream = fs.createWriteStream(FIFO_ROCK_UNROCK);
			wstream.write(JSON.stringify({rock_state: d.rock_state}) + '\n');
			io.sockets.emit('rock_unrock/update', d);
		});
